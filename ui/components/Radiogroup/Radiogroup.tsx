      ...
      {options?.map(({ id, value, label }) => (
        <div className={`flex items-center`} key={id}>
          <RadixRadioGroup.Item
            className="bg-surface-white w-6 h-6 rounded-full transition duration-300 shadow hover:bg-surface-50 focus:shadow-xl"
            value={value}
            id={id}
          >
          ...
          <label className="pl-3.5 text-md select-none text-surface-40" htmlFor={id}>
            {label}
          </label>
        </div>
      ))}
